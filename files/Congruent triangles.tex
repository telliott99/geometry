...
There is one set of three that doesn't work in the general case, that is SSA (side-side-angle).  But don't just dismiss it, because it does work sometimes.
...
There are two possibilities, not just one, so we say that the triangle is not determined.  And if two different triangles match the SSA criterion, we cannot always say whether they are congruent or not.  
