...
Quite often, we will label the side opposite a vertex with a lower case letter:  side $a$ lies opposite $\angle A$.  Sometimes we will use letters like $\theta$ and $\phi$ for angles, or even $s$ and $t$.
...
$\bullet$ \ In an isosceles triangle (two sides equal), then the base angles are also equal.
...
Imagine the triangle sticks straight up from the plane like one of those standing stones at Stonehenge.  Think about walking around the triangle and looking from behind, it would look the same.  Then we could say that the left side as viewed from the front is equal to the right side as viewed from the front, because if we walk around behind the triangle the right side becomes the \emph{left} and vice-versa.
...
Next, we will prove that the SSS criterion implies SAS.

\emph{Proof}.

Let all the sides of $\triangle ABC$ be equal to $\triangle DEF$.  Choose the longest side of $\triangle DEF$ and align it with $\triangle ABC$ as shown.
...
To make the drawing, we have chosen a particular value for the total angle at vertex $C$.  A similar diagram can be drawn starting with a right triangle or an obtuse triangle.  Therefore the proof works for all triangles.  
