...
Here are fuive more propositions from Euclid's first book.  They are short, sweet and powerful.  

\subsection*{Prop. I.7}

\label{sec:Euclid7}

\begin{center} \includegraphics [scale=0.4] {Euclid7a.png} \end{center}

Let $\triangle ABD$ be drawn and a point $C$ be chosen on the same side of $AD$ as $B$ lies.

It cannot be true that both $AB = AC$ and $DB = DC$.  (Note:  if it were true then the two triangles would be congruent by SSS, which Euclid gets to just after this proposition).

\emph{Proof}.

Suppose that both statements are true: $AB = AC$ and $DB = DC$.  Then $\triangle ABC$ is isosceles so $\angle ABC = \angle ACB$.  For the same reason, $\angle DBC = \angle DCB$.

By construction:  $\angle ABC > \angle DBC$, and from above $\angle DBC = \angle DCB$.  Also by construction:  $\angle DCB > \angle ACB$.

Chaining them together we see that  $\angle ABC > \angle ACB$.

But above we had:  $\angle ABC = \angle ACB$.

This is a contradiction.  Therefore it cannot be that both $AB = AC$ and $DB = DC$.

$\square$

In the other four examples, we use letters late in the alphabet ($s, t, u, v$) for angles, while $a, b, c$ are labels for sides.
...
Clearly, if a given side is not the longest, i.e. it is shorter than one of the others, then it must also be shorter than the sum of that other one plus the third.  For this reason, we consider only the longest side.
