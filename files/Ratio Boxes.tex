...
But looked at another way, they have bases $BD$ and $DC$ on the same parallel, so they have the same height drawn to vertex $C$.  Hence, the areas are in the ratio $BD:DC$.  Thus
\[ \frac{AB}{AC} = \frac{BD}{DC} \]

\subsection*{Menelaus's Theorem}
The next example is one where the method really proves its worth.  We prove Menelaus's theorem (we will look at it in more detail later \hyperref[sec:Menelaus_theorem]{\textbf{here}}) by drawing an internal line segment parallel to one of the sides.
\begin{center} \includegraphics [scale=0.18] {ratios14.png} \end{center}

We have decided to label vertices rather than sides. We do not worry about the order in which they are listed (compare $BQ$ to $QC$).  Just carefully list the sides in the correct order around each triangle, starting at the same vertex each time (since we have parallel bases). 

Notice that the values in the right-hand column are all subtractions, but for one there is no labeled segment to equate to the result.  Now, identify the segments we need for the proof.
\begin{center} \includegraphics [scale=0.18] {ratios15.png} \end{center}
It writes itself!
\[ BR \cdot \frac{AP}{AR} = PK = CP \cdot \frac{BQ}{QC} \]
\[\frac{BQ}{QC}  \cdot  \frac{CP}{AP} \cdot \frac{AR}{BR} = 1 \]

$\square$

If you wish, reverse some segments to get the standard orientation:
\[\frac{BQ}{QC}  \cdot  \frac{CP}{PA} \cdot \frac{AR}{RB} = 1 \]
Note that one of these ratios is "negative" (the two segments point in opposite directions), and adjust the sign of the result accordingly.
\[\frac{BQ}{QC}  \cdot  \frac{CP}{PA} \cdot \frac{AR}{RB} = -1 \]
