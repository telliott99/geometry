...
By the definition of radian measure, a central angle is equal numerically to the arc it subtends on a unit circle.  

A vertex placed \emph{at any point} on the periphery of a circle, forming an angle that subtends the same arc as a central angle, is equal to one-half the central angle.

A corollary is that whenever two peripheral angles subtend the same arc, they are equal.

\subsection*{proof based on external angle theorem}

\label{sec:equal_angles_same_arc}

There is a proof in Acheson's geometry book, which invokes the external angle theorem. 
...
A slight modification is needed for the case where the arc does not include the center of the circle. Then we would have $PA$ and $PB$ on the same side of the diagonal, and the relevant arc would be that subtended by $a - b$ on the periphery, or $2(a-b)$ at the center.

This result does not depend on where $P$ is placed.  Therefore all peripheral angles subtending the same arc are equal.  So we get the general case with only one proof.
...
Here is a different proof of the previous theorem, which invokes Thales' theorem about right angles in a semicircle (\hyperref[sec:Thales_theorem]{\textbf{here}}).
...
We previously established some central facts about circles including Thales' theorem:  the angle intercepting a half-arc of the circle is a right angle.

We also showed, based on the fundamental theorem, that if an angle $s$ is formed on the perimeter of a circle with one of its adjacent sides being a diagonal of the circle, then it intercepts an arc $a$ where

\[ 2s = a \]  
\[ s = \frac{1}{2} \ a \]

Next, we generalize the result.  (The next few proofs are alternatives to the proof of the inscribed angle theorem given above).
...
$\bullet$ \ Any angle on the perimeter of a circle subtends half the arc that it cuts out of the circle.
...
(Alternatively, the supplementary angle to $u$ cuts off everything except $b$).
...
We showed previously that when two tangents are drawn from an exterior point, one can draw two right triangles that share the hypotenuse and have another side equal to the radius, so they are congruent (SSA in a right triangle).
...
\[ \frac{\phi}{2} + \frac{\theta}{2} = \frac{\pi}{2} \]
\[ 2 \phi = 2 \pi - 2 \theta \]
...
Alternatively, just say that, since the other two angles in the quadrilateral are right angles
\[ \phi + \theta = \pi \]

but $\theta = s$ and $2 \pi = s + t$ so
\[ \phi = \pi - \theta = \frac{1}{2} (s + t) - s \]
\[ = \frac{1}{2} (t - s) \]
