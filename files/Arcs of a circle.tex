...
An angle \emph{anywhere} on the periphery that subtends the same arc as a central angle is equal to one-half the central angle.
...
A slight modification is needed if the arc does not include the center of the circle. Then we would have $PA$ and $PB$ on the same side of the diagonal, and the relevant arc would be that subtended by $a - b$ on the periphery, or $2(a-b)$ at the center.
...
Next, we generalize the result.  (The next few proofs are an alternative to the proof of the inscribed angle theorem given above).
