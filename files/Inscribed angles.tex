...
In this chapter, we introduce the inscribed angle theorem.  Let's start by revisiting the last of Thales' theorems:

$\bullet$  Any angle inscribed in a semicircle is a right angle.

Take any three points on the circumference of the circle, forming a triangle. If two of the points are on a diameter of the circle, the angle formed at any arbitrary but distinct third point is always a right angle.
...
Note:  this theorem is Euclid Prof. III.31.  Rather than arithmetic, he uses the external angle theorem to reach the result.  

According to Boyer, this result was known to the Egyptians 1000 years before Thales.  This is yet another example of knowing a result before proving that it is so.  Archimedes had something to say about the importance of having discovered a fact, before knowing how to prove it.
...
The right angle on the perimeter of the circle which is corresponds to one-half of the circle is
\[ \angle PRQ = \pi/2 \]

The arc is $\pi$ but the angle is $\pi/2$.  What's going on?
...
This is clear in the special case where one arm of the inscribed angle is a diameter of the circle, but it turns out to be generally true, as we'll see below.
...
An angle on the perimeter of a circle is generally called an \emph{inscribed} angle.  One can view an inscribed angle as part of a triangle whose vertices are all on the circle.  The following is called the \emph{inscribed angle theorem}.

$\bullet$  An inscribed angle is twice the central angle corresponding to the same arc.
...
A different method is to start with the result for an angle where one adjacent side is a diameter of the circle.  Any other peripheral angle can be constructed as either the sum or the difference of two such constructs.  We'll show that later on.

\subsection*{theorem on cyclic quadrilaterals}

\label{sec:quadrilateral_supplementary}

All this leads to a wonderful simple theorem about quadrilaterals.  A cyclic quadrilateral is a four-sided polygon whose four vertices all lie on one circle.
...
The relationships that seem obvious from the diagram are given.  Namely, $PXYR$ and $QXZS$ are colinear, and the two circles each have the four points lying on them as shown.  $TPS$ is tangent to the smaller circle at $P$.
