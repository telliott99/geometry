...
Since the figure is a rectangle, you can read the relevant formulas off the figure.
...
There is a neat geometric derivation of this formula.  Draw a right triangle with its hypotenuse along the diagonal in a circle of diameter 1.  The sides of this right triangle are just $\sin \theta$ and $\cos \theta$.

Now drop the altitude (in red) to make two smaller right triangles still with angle $\theta$.  This triangle has one side equal to $ \cos^2 \theta$, since $\cos^2 x/\cos x = \cos x$.  The red vertical is $\sin \theta \cos \theta$.

\begin{center} \includegraphics [scale=0.5] {half_angle_3.png} \end{center}

The second part of the hypotenuse along the diameter is equal to $\sin^2 \theta$, since all three right triangles are similar.  This is a quick proof of the Pythagorean theorem.

By the properties of inscribed angles, the central angle is $2 \theta$, and its cosine is the part of hypotenuse lying between the center of the circle and the base of the altitude, times by $1/2$, so that after dividing by the hypotenuse (equal to $1/2$), we obtain our result.
