...
Previously, we gave several ways to prove that two triangles are congruent.  These four methods (SAS, SSS, ASA, and AAS) are also useful with right triangles.  Some books give them new names in the context of a right triangle.
...
These are not worth much.  However, there are some new considerations for right triangles.
...
It is worth pointing out that if we have SSA not in a right triangle, then for an appropriate length of $b$ there are only two possibilities, as illustrated.  So if we can further specify whether $b$ makes with the third side is acute or obtuse, we may be able to show congruence.
...
We showed above that in any right triangle, the right angle is larger than either of the other two angles.  By Euclid \hyperref[sec:Euclid1]{\textbf{Prop $I.18$}}:  in any triangle, a greater side is opposite a greater angle.  

$\square$

Or we look ahead again to the Pythagorean theorem.  Since $a > 0$ and $b > 0$ and 
\[ c^2 = a^2 + b^2 \]
Suppose $b > a$.  It follows that $c^2 > b^2$ so $c > b > a$.
