...
A circle can be drawn with a compass, as we mentioned previously.
\begin{center} 
...
The point $O$ is the circumcenter of the circumcircle that includes all three vertices of a triangle, here the $\triangle QRS$.  In this example, the triangle is obtuse (it contains an angle greater than a right angle), so the origin of the circumcircle is not inside the triangle.

\subsection*{Prop.III.4}

\label{sec:Euclid_III_1}
...
If there is no specification of an internal point, then it is possible for a line to intersect with the circle at only one point, namely a tangent line.  We deal with tangents in a separate chapter.
...
Central angle and the arc that subtends that angle are numerically equal, but remember that they are dimensionally different.  Arc is a length, angle is just an angle.
