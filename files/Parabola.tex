...
If $p = 1/4$, then $y = x^2$.  For a larger $p$, the constant factor $1/4p$ gets smaller, meaning that $y$ will be smaller for a each corresponding $x$, so the parabolas' arms become more shallow.
...
The slope of the tangent is simply the ratio of these $y-$ and $x-$ components.  $\Delta y/\Delta x = 2y/x = 2x^2/4px = x/2p$.  For a parabola defined as $y = ax^2$, the slope is $2ax$.  As you will see later on, this result is literally the first result in calculus.
...
By the definition of the parabola, any point $P$ on the parabola lies on a vertical up from $D$ such that $PF = PD$.  That equality means that the same point $P$ must lie on the vertical bisector of $FD$, by the properties of isosceles triangles.
...
Suppose the perpendicular bisector is not the tangent.  Then, perhaps it would not cut the parabola at all.  But this would mean that no point on the parabola would satisfy the invariant.  And we know that $PI$ is not parallel to $PD$, so they must cross.

\begin{center} \includegraphics [scale=0.35] {para_geo_3.png} \end{center}

Suppose the perpendicular bisector is a secant and cuts through two points on the parabola.  In that case, the second point has two contradictory properties.

On one hand, $P'$ is on the bisector.  It is therefore equidistant from $F$ and $D$.
