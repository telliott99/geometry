...
There are several different conventions for referring to the area of a triangle.  One is just to use the capital letter $A$ (for area):
\[ A_{\triangle_{PDE}} = A_{\triangle_{QDE}} \]

That becomes awkward when $A$ has another meaning in the problem.  Some people switch to using $K$, but a second approach is to use the $\triangle$ symbol, as in
...
Euclid's approach is detailed in three propositions in Book I of \emph{Elements}.  Here is $I.35$.  This proposition (theorem) says that given parallelograms $ABCD$ and $EBCF$ on the same base $BC$ and with $ADEF \parallel BC$, they have the same area.
...
\[ 2A = \frac{a^2}{2} = \frac{xa}{2} + \sqrt{3} xa \]
\[ a = x + 2 \sqrt{3} x \]
\[ x = \frac{a}{1 + 2 \sqrt{3}} \]
