...
This is first of all a statement about the world, that we can have two lines that run alongside each other but don't touch.  Another way to talk about this situation is to say that if we sight down a pair of parallel lines from left to right, or turn the paper $180^{\circ}$ and look again, the picture should look the same.

By rotational symmetry then, we have no reason to distinguish two parallel lines in the forward and reverse directions.  It follows that the intersection between two parallel lines and a third line that crosses them should look the same in both directions.  That is our theorem.  
...
This definition works for geometry in the flat plane, but not on a curved surface like the earth.  That's a familiar situation where our postulate is not appropriate.
...
The same issue holds if you imagine the earth at the center of the universe, looking out at the stars.  Or place yourself inside a globe, looking at  the thin skin of the object and thinking about the lines of latitude and longitude as seen from the inside.
...
Technically, it is only the two angles in the left panel that are marked with black dots, that are equal by the theorem of alternate interior angles.  The others in the middle panel are equal by the vertical angles theorem.  We will not usually distinguish these cases, and just refer to both of them as "equal by alternate interior angles."
...
We come to our first truly novel theorem, that relies on everything we've said so far.  It is attributed to Thales, one of three elementary but novel theorems for which he is thought to have developed proofs.
...
Heath says that rather than Thales, Pythagoras is responsible for the triangle sum proof.
...
This is simply addition and subtraction, once you understand about supplementary angles and the sum of angles theorem.  Go ahead and do that.  

But then, how about proving a new theorem on your own?  Try it.  We will give the theorem in a later chapter, just in case.

$\angle u'$ is called the \emph{external angle} of this triangle.  It is related to $\angle s$ and $\angle t$.  Write an equation that gives the relationship between the three angles.

\subsection*{another proof}
Here is a different proof of both of these theorems relating to the angles in a triangle.  I learned this proof from Alcock (\emph{Mathematics Rebooted}).
...
Imagine walking around the perimeter of a triangle in the counter-clockwise direction.  At each vertex we turn left by a certain angle, $\theta$, called the exterior angle.  After passing through all three vertices, we must end up facing in the same direction as we started.

The sum of the exterior angles is $4$ right angles, and we have made one complete turn.
