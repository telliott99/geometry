...
This chapter involves angle bisectors in an isosceles triangle.  We called the forward version the \hyperref[sec:isosceles_bisector]{\textbf{isosceles bisector}} theorem.
...
We are given that $\triangle ABC$ is isosceles ($AC = BC$), and also that the angles at the base are both bisected.  We showed previously that the angle bisectors are equal in length:  $AF = BD$.
...
We prove both these theorems \hyperref[sec:Euclid18]{\textbf{here}}.  We also need the following lemma.  
...
Similarly, $\phi > \theta \Rightarrow d > c$  $\square$
...
Draw $CD$.  Comparing $\triangle CBD$ and $\triangle DYC$, we have that $CD$ is shared, $DY = BC$ by construction, $\angle CYD = \angle CBD$ (SSA) and also, the angle between the two known sides must be acute in both, since $\angle CYD$ and $\angle CBD$ are obtuse.
...
Here is another proof of the theorem.  First we revisit Stewart's \hyperref[sec:Stewart_bisected]{\textbf{theorem}} in the case of the bisected angle, from the previous chapter.  Substitute the symbol $t$ (for traversal).
\begin{center} \includegraphics [scale=0.15] {steiner1.png} \end{center}
\[ ab = t^2 + xy \]
\[ t^2 = ab - xy \]

This is a general formula for the length of the traversal when it is an angle bisector.  We need to eliminate the $xy$ term.  One way to write the fundamental relationship for a bisected angle is
\[  \frac{a}{b} = \frac{y}{x} \]
...
The length (squared) of the traversal to side $c$ is symmetric in $a$ and $b$, as it should be.

\[ = ab \ [ \ 1 - \frac{c^2}{(a+b)^2} \ ] \]
\[ = ab \ [ \ \frac{(a + b)^2 - c^2}{(a+b)^2} \ ] \]
\[ = ab \ [ \ \frac{(a + b + c)(a + b - c)}{(a+b)^2} \ ]  \]

We can apply this formula to the main problem, since there are two traversals which are equal.  The algebra gets pretty messy.  I could direct you to
...
In fact, I put it there.  This proof is probably due to Lehmus.

There is some interesting discussion in Coxeter as well.  Much of the literature concerns the question of whether it is possible to provide a direct proof of the theorem.  The last example we gave above, the algebraic proof, has been cited as such.  

However, that algebraic proof depends on Stewart's Theorem, which as we derived it depends on the Law of Cosines, which depends in turn on the theorem of Pythagoras.  And although there are several hundred proofs of Pythagoras most (all?) of them depend on the sum of angles and also on the parallel postulate, which explicitly depends on a proof by contradiction.  

The question of a direct proof for Steiner-Lehmus is hard to answer conclusively.  There are proofs claiming that it is impossible.
