...
We'll actually use some words in an attempt to see how one might have developed this proof.

\begin{center} \includegraphics [scale=0.5] {pt11.png} \end{center}

Check first that we have the labels correct:  a (red), b(blue), c(magenta) and d(green).

The idea is to construct a parallelogram.  We begin by cutting the figure in half along one of the diagonals, say $m$.

\begin{center} \includegraphics [scale=0.5] {pt12.png} \end{center}

Next, pick any two sides and align them along a horizontal line.  

Draw a second line across the top, parallel to the bottom, scaling one triangle as necessary to make it fit.  (Our drawing isn't perfect, so the left and right sides look a little off, but all the angles will check out at the end).

\begin{center} \includegraphics [scale=0.5] {pt13.png} \end{center}

We can fill in the angles needed to make a parallelogram using the alternate interior angles theorem.

\begin{center} \includegraphics [scale=0.5] {pt14.png} \end{center}

If this is going to be a parallelogram, the left and right sides must be equal in length.  We achieve this by scaling each of the two original triangles, using the opposing sides as the scale factor.

\begin{center} \includegraphics [scale=0.5] {pt15.png} \end{center}

Suddenly, we realize that we have a matching triangle in the original figure.  It just needs to be scaled by $m$ to fit.  The red and green dotted angles are switched (the large triangle is inverted), but that's no problem.

\begin{center} \includegraphics [scale=0.5] {pt16.png} \end{center}
