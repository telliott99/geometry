...
If it is to be equal, then it is (again) natural to wonder that if $AC$ is equal to $CD$ plus $BC$, perhaps laying off the shorter chord on the big piece of the longer one gives any insight.  We will see that both possible points of division do so.
...
This is a basic result.  There is one perpendicular bisector for any two parallel chords in a circle.  As a result, if a rectangle has two points lying on the circle, if the opposing side is extended to meet the circle, the extensions are also equal.  We leave this simple proof as an exercise.
