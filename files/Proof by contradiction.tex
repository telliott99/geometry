...
This chapter introduces proof by contradiction.  In Latin it is called \emph{reductio ad absurdum}.  We start with four simple proofs, for problems which are not really related to geometry, but provide insight into the method.  They are all very famous.  
...
The fraction of numbers which is prime (the density of primes) decreases as the integers get larger.  There are 25 primes smaller than 100, yet later on the density is substantially less.  
...
For $k$ equal to one million, $\ln k \approx 13.8$ so $\pi(k)$ is about 7\% of $k$.  For $k$ equal to one trillion, it's half that.
...
(Note:  this proof does not show that $N$ is the next prime greater than $p_n$.  For example, $2 \cdot 3 + 1 = 7$, which is prime, but $7$ is not the next prime after $3$.)
...
For $\triangle ABC$, if we are given that $AB = AC$, then it follows that the base angles at vertices $B$ and $C$ (marked with black dots) are equal.  The proof (which we gave previously) involves drawing the angle bisector from $A$, the equal angles being marked with an $x$.  

Then, we have SAS, since $AB = AC$, $\angle BAD \cong \angle DAC$ and $AD = AD$ (of course).  It follows that $AD$ is not only the angle bisector, but also the altitude and the median of the original triangle.
...
Another nice proof of this is given in Acheson.
...
$\circ$  Then $\angle MCB < \angle B$ (by $I.18$ converse).
